...
(localData.users || []).forEach((u: any) => { userMap[u.id] = { name: u.name, email: u.email }; });
...
for (const line of relevantLines) {
    try {
        ...
        if (description.toLowerCase().includes('login')) actionType = 'login';
        else if (description.toLowerCase().includes('logout')) actionType = 'logout';
        else if (description.toLowerCase().includes('export') || description.toLowerCase().includes('word') || description.toLowerCase().includes('excel')) actionType = 'export';
        else if (event === 'Created') actionType = 'create';
        else if (event === 'Updated') actionType = 'update';
        else if (event === 'Deleted') actionType = 'delete';
        else if (logName === 'Access') actionType = 'access';
...
console.log(`Tipos de eventos:`, [...new Set(logs.map((l: any) => l.type))]);
