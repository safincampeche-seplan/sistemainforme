...
async function run() {
    ...
    const allPeriods = await (prisma as any).cat_periods.findMany({ orderBy: { id: 'asc' } });
    allPeriods.forEach((p: any) => console.log(c.info(`  ID:${p.id} → "${p.name}"`)));

    // 2. Buscar el periodo más reciente anterior a 2026 que tenga datos
    console.log(c.head('BUSCANDO PERIODO CON MÁS DATOS'));
    let sourcePeriod: any = null;
    let maxEntries = 0;

    for (const p of allPeriods) {
        if (p.name === '2026') continue; // Saltar el 2026
        const count = await (prisma as any).entry.count({ where: { period_id: p.id } });
    ...
    let period2026 = await (prisma as any).cat_periods.findFirst({ where: { name: '2026' } });
    if (!period2026) {
        period2026 = await (prisma as any).cat_periods.create({
    ...
    const sourceEntries = await (prisma as any).entry.findMany({
    ...
    const firstUser = await (prisma as any).user.findFirst({ where: { is_active: true } });
    ...
    for (const entry of sourceEntries) {
        try {
            ...
            if (entry.values?.length > 0) {
                const valueData = entry.values.map((v: any) => ({
                ...
                const entityName = await (prisma as any).entity.findUnique({
    ...
    if (skipped > 0) console.log(c.warn(`  ${skipped} ya existían en 2026 (no duplicadas)`));
    if (errors > 0) console.log(c.fail(`  ${errors} errores`));
