...
async function importUsers() {
    ...
    for (const line of lines) {
        if (!line.includes('INSERT INTO `users` VALUES '))
            continue;
        ...
        for (const row of rows) {
            ...
            if (cols.length >= 6) {
                ...
                // Decide role simply
                let role = 'Capturista';
                if (email.includes('super') || name.toLowerCase().includes('admin'))
                    role = 'SuperAdministrador';
                else if (name.toLowerCase().includes('validador'))
                    role = 'Validador';
...
importUsers();
//# sourceMappingURL=import_users.js.map