...
const c = {
    ...
    info: (s: string) => `\x1b[36mℹ️  ${s}\x1b[0m`,
    head: (s: string) => `\x1b[1m\x1b[35m\n${'─'.repeat(52)}\n ${s}\n${'─'.repeat(52)}\x1b[0m`,
};
...
async function run() {
    ...
    let entities: any[] = [];
    try {
        entities = await (prisma as any).entity.findMany({
        ...
        if (entities.length === 0) {
            ...
            for (const e of entities) {
                ...
                const hasMockIds = e.properties?.some((p: any) => Number(p.id) < 100);
                if (hasMockIds) {
    ...
    const testEntity = entities.find((e: any) => (e.properties?.length || 0) > 0);
    if (!testEntity) {
    ...
    if (!testEntity?.properties?.length) {
        ...
        testEntity.properties.slice(0, 8).forEach((p: any) => {
    ...
    try {
        periodRecord = await (prisma as any).cat_periods.findFirst({
            where: { name: '2026' }
        });
        if (!periodRecord) {
    ...
    let createdEntryId: bigint | null = null;
    ...
    if (testEntity?.properties?.length && periodRecord) {
        ...
        try {
            const u = await (prisma as any).user.findFirst({ where: { is_active: true } });
            const realUserId = u ? u.id : BigInt(1);

            // 1. Crear el Entry (fila)
            const newEntry = await (prisma as any).entry.create({
                data: {
            ...
            const valueData = testEntity.properties.slice(0, 5).map((p: any) => ({
            ...
            await (prisma as any).value.createMany({ data: valueData });
    ...
    if (createdEntryId) {
        try {
            const retrieved = await (prisma as any).entry.findUnique({
            ...
            if (!retrieved) {
                ...
                retrieved.values?.forEach((v: any) => {
    ...
    if (createdEntryId) {
        try {
            await (prisma as any).value.deleteMany({ where: { entry_id: createdEntryId } });
            await (prisma as any).entry.delete({ where: { id: createdEntryId } });
    ...
    if (errors === 0 && warnings === 0) {
        ...
        if (errors > 0) console.log(c.fail(`${errors} error(es). Ver detalles arriba.`));
        if (warnings > 0) console.log(c.warn(`${warnings} advertencia(s). Ver detalles arriba.`));
    }
    console.log('');
