...
async function v() {
    ...
    for (const period of periods) {
        const missionsCount = await p.mission.count({ where: { narrative_period_id: period.id } });
        console.log(`Period ID ${period.id} (${period.year}) - Missions: ${missionsCount}`);
    }
    const allMissions = await p.mission.findMany({ take: 5 });
...
//# sourceMappingURL=full_scan.js.map