...
async function main() {
    ...
    let period = await (prisma as any).cat_narrative_periods.findFirst({
        where: { year: '2026' }
    });

    if (!period) {
        period = await (prisma as any).cat_narrative_periods.create({
            data: {
                year: '2026',
    ...
    const titleMap = new Map<string, any>(); // key: mission_id-title_id
    const themeMap = new Map<string, any>(); // key: title_id-theme_id
    const subthemeMap = new Map<string, any>(); // key: theme_id-subtheme_id

    // 2. Import Missions, Titles, Themes, Subthemes
    for (const item of indiceData) {
        // Mission
        if (!missionMap.has(item.id_mision)) {
            let m = await (prisma as any).mission.findFirst({
                where: { name: item.descripcion_mision, narrative_period_id: periodId }
            });
            if (!m) {
                m = await (prisma as any).mission.create({
                    data: {
        ...
        if (!titleMap.has(titleKey)) {
            let t = await (prisma as any).narrativeTitle.findFirst({
                where: { name: item.descripcion_titulo, mission_id: mission.id }
            });
            if (!t) {
                t = await (prisma as any).narrativeTitle.create({
                    data: {
        ...
        if (!themeMap.has(themeKey)) {
            let th = await (prisma as any).narrativeTheme.findFirst({
                where: { name: item.descripcion_tema, narrative_title_id: title.id }
            });
            if (!th) {
                th = await (prisma as any).narrativeTheme.create({
                    data: {
        ...
        if (!subthemeMap.has(subthemeKey)) {
            let st = await (prisma as any).cat_narrative_sub_themes.findFirst({
                where: { name: item.descripcion_subtema, narrative_theme_id: theme.id }
            });
            if (!st) {
                st = await (prisma as any).cat_narrative_sub_themes.create({
                    data: {
    ...
    for (const name of beneficiaries) {
        const exists = await (prisma as any).cat_narrative_beneficiary_types.findFirst({ where: { name } });
        if (!exists) {
            await (prisma as any).cat_narrative_beneficiary_types.create({ data: { name } });
        }
    }
    ...
    for (const name of sources) {
        const exists = await (prisma as any).cat_narrative_financing_sources.findFirst({ where: { name } });
        if (!exists) {
            await (prisma as any).cat_narrative_financing_sources.create({ data: { name } });
        }
    }
