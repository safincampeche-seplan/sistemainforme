...
async function fixTrigger() {
    try {
        ...
        await prisma.$executeRawUnsafe(`DROP TRIGGER IF EXISTS trigger_after_update_narrative_capture;`);
...
BEGIN
    IF NEW.status = 'approved secont' AND (OLD.status != 'approved secont' OR OLD.status IS NULL) THEN
        ...
        INSERT INTO narrative_captures_second_final (
            narrative_capture_id, sequence_number, narrative_period_id, dependency_id, 
            ...
            narrative_breakdown, highlighted, stage, created_by, edited_by, 
        ...
        VALUES (
            ...
            NEW.narrative_breakdown, NEW.highlighted, NEW.stage, NEW.created_by, NEW.edited_by, 
            NEW.deleted_at, NEW.created_at, NEW.updated_at
        );

        INSERT INTO entity_linkage_narrative_second_final (entity_id, narrative_capture_id) 
        SELECT entity_id, narrative_capture_id FROM entity_linkage_narrative 
...
fixTrigger();
//# sourceMappingURL=fix_trigger.js.map