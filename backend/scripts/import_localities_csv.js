...
async function main() {
    ...
    const munData = importCSV('cat_municipalities.csv');
    ...
    const locData = importCSV('cat_localities.csv');
    let count = 0;
    for (const row of locData) {
        try {
            ...
            count++;
            if (count % 500 === 0)
                console.log(`Progreso: ${count} localidades...`);
        }
        catch (err) {
...
//# sourceMappingURL=import_localities_csv.js.map