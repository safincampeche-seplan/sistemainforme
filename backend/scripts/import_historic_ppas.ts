...
async function main() {
    ...
    for (const row of data) {
        try {
            ...
            // 3. Estatus
            let status: narrative_captures_status = narrative_captures_status.historical__revised_;
            if (row['ESTATUS']?.toString().toLowerCase().includes('borrador')) status = narrative_captures_status.draft;
            if (row['ESTATUS']?.toString().toLowerCase().includes('validación')) status = narrative_captures_status.under_validation_semaig;
            if (row['ESTATUS']?.toString().toLowerCase().includes('aprobado')) status = narrative_captures_status.approved_secont;
            ...
            count++;
            if (count % 10 === 0) console.log(`Procesados ${count} registros...`);
        } catch (err) {
