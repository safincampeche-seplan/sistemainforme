...
async function main() {
    ...
    const dataIndice: any[] = xlsx.utils.sheet_to_json(sheetIndice);
    ...
    for (const row of dataIndice) {
        const {
            id_mision, descripcion_mision,
            id_titulo, descripcion_titulo,
            id_tema, descripcion_tema,
            id_subtema, descripcion_subtema
        } = row;

        if (!id_mision) continue;
    ...
    const dataProg: any[] = xlsx.utils.sheet_to_json(sheetProg, { range: 3, header: ['code', 'name'] });
