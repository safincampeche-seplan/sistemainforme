...
async function runEndToEndFlow() {
    ...
    try {
        ...
        if (!capturistaRole) throw new Error("Role Capturista not found");

        const userHasRole = await prisma.userHasRole.findFirst({
            where: { role_id: capturistaRole.id }
        });

        if (!userHasRole) throw new Error("No users with Capturista role found");
        ...
        if (!capturista) throw new Error("Capturista user not found");

        userEmail = capturista.email;
        dependencyName = capturista.dependency?.name || 'N/A';

        token = jwt.sign(
            { id: capturista.id.toString(), email: capturista.email, roles: ['Capturista'], name: capturista.name, dependency_id: capturista.dependency_id?.toString() },
            process.env.JWT_SECRET || 'seplan_secret_key_2024_secure',
            { expiresIn: '1h' }
        );
    ...
    try {
        ...
        const found = myNarratives.find((n: any) => n.id.toString() === savedNarrativeId.toString());
        if (found) {
