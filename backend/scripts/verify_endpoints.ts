...
async function runTests() {
    ...
    try {
        ...
        if (!adminUser) throw new Error("No active users found");

        token = jwt.sign(
            { id: adminUser.id.toString(), email: adminUser.email, roles: ['SuperAdministrador'], name: adminUser.name },
    ...
    const testEndpoint = async (method: 'get' | 'post', path: string, data?: any) => {
        try {
