...
async function v() {
    ...
    // 1. Check Period
    const period = await (p as any).cat_narrative_periods.findFirst({ where: { year, deleted_at: null } });
    console.log('Active 2026 Period ID:', period ? period.id.toString() : 'NONE');

    if (!period) return;

    // 2. Check Titles for either ID 1 or the found ID
    const titles = await (p as any).narrativeTitle.findMany({
    ...
    if (titles.length === 0) {
        ...
        const sampleTitle = await (p as any).narrativeTitle.findFirst({
    ...
    console.log('p.cat_narrative_financing_sources:', !!(p as any).cat_narrative_financing_sources);
    console.log('p.cat_narrative_beneficiary_types:', !!(p as any).cat_narrative_beneficiary_types);
