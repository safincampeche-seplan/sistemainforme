...
async function logActivity(user, event, module, description, req) {
    try {
...
const notifyStatusChange = async (narrativeId, newStatus, observations) => {
    try {
        const narrative = await prisma.narrativeCapture.findUnique({
...
const authorize = (allowedRoles) => {
    return (req, res, next) => {
        ...
        const hasPermission = normalizedUserRoles.some((role) => normalizedAllowedRoles.includes(role));
        if (!hasPermission) {
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        const userRolesRes = await prisma.$queryRawUnsafe(`
            SELECT r.name 
        ...
        const normalizedRoles = roles.map(r => {
            ...
            if (lower === 'superadministrador' || lower === 'super_admin')
                return 'super_admin';
        ...
        console.log(`Login Successful: ${userFound.email} | Roles: ${normalizedRoles} | Mission: ${userFound.dependency?.mission_id}`);
        res.json({
            token,
            user: {
                id: userFound.id.toString(),
                ...
                mission_id: userFound.dependency?.mission_id?.toString() || null
            }
        });
    }
    catch (error) {
...
app.get('/api/dashboard/stats', authenticateToken, async (req, res) => {
    try {
        ...
        const selectedYear = periodo || '2026';
        ...
        const iscapturistaOnly = user && user.roles &&
            user.roles.some((r) => r.toLowerCase().includes('capturista')) &&
            !user.roles.some((r) => ['super_admin', 'superadministrador', 'admin'].includes(r.toLowerCase()));
        ...
        const approvedNarratives = narratives.filter((n) => ['approved_secont', 'finalized', 'historical__revised_'].includes(n.status)).length;
        ...
        const totalBeneficiaries = narratives.reduce((sum, n) => sum + (Number(n.beneficiaries) || 0), 0);
        ...
        res.json({
            ...
            completedPercent,
            pendingValidations: narratives.filter((n) => n.status.includes('under_validation')).length,
            totalEntities,
            capturedEntities,
            pendingEntities,
            totalUsers
...
app.get('/api/dashboard/search', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const narrativeMatches = await prisma.narrativeCapture.findMany({
            where: {
...
app.get('/api/activities', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const activities = await prisma.narrativeCapture.findMany({
            where: whereClause,
            take: 10,
...
app.get('/api/entities', authenticateToken, async (req, res) => {
    ...
    const selectedPeriod = parseInt(periodo) || 2026;
    ...
    try {
        ...
        if (isAdmin) {
            if (sector_id) {
                where.dependency = { sector_id: BigInt(sector_id) };
            }
            if (mission_id) {
                where.dependency = { ...where.dependency, mission_id: BigInt(mission_id) };
            }
        }
...
app.post('/api/entries', authenticateToken, authorize(['super_admin', 'admin', 'capturista']), async (req, res) => {
    ...
    try {
        ...
        let periodRecord = await prisma.cat_periods.findFirst({
...
app.post('/api/admin/entities/:id/import', authenticateToken, upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            // Eliminar anteriores
            const existingEntries = await tx.entry.findMany({
                where: { entity_id: BigInt(id), period_id: periodRecord.id }
            });
            if (existingEntries.length > 0) {
                const entryIds = existingEntries.map((e) => e.id);
                await tx.value.deleteMany({ where: { entry_id: { in: entryIds } } });
...
app.get('/api/entries/:entityId', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // 1. Intentar desde DB
        const periodRecord = await prisma.cat_periods.findFirst({
            where: { name: selectedPeriod.toString() }
        });
        if (periodRecord) {
            ...
            if (dbEntries.length > 0) {
                ...
                const uiRows = dbEntries.map((entry) => {
                    const row = { id: entry.id.toString() };
...
app.get('/api/catalogs/sectors', authenticateToken, async (req, res) => {
    try {
        ...
        res.json(sectors.map((s) => ({ ...s, id: s.id.toString() })));
    }
    catch (err) {
        res.json([]);
    }
...
app.get('/api/tracking/all', authenticateToken, async (req, res) => {
    ...
    const mapStatus = (rawStatus) => {
        ...
        if (s.includes('draft') || s === 'borrador')
            return 'Borrador';
        if (s.includes('observed') || s.includes('observaciones') || s.includes('with_observations'))
            return 'Con Observaciones';
        if (s.includes('approved') || s.includes('aprobado') || s === 'historical')
            return 'Aprobado';
        return 'Completado'; // Default pending validation (under_validation, finalized, en proceso)
    };
    try {
        const periodId = selectedPeriod === 2026 ? 5 : 4;
        let narrativeTracking = [];
        let statisticsTracking = [];
        ...
        try {
            const dbNarratives = await prisma.narrativeCapture.findMany({
            ...
            narrativeTracking = dbNarratives.map((n) => ({
                id: Number(n.id),
                type: 'narrativa',
            ...
            console.error("Error DB Narratives Tracking", err);
        }
        ...
        try {
            ...
            console.error("Error DB Entities Tracking", err);
        }
        let combined = [...narrativeTracking, ...statisticsTracking];
        if (dependencyId) {
            combined = combined.filter(c => c.entity.includes(dependencyId));
        }
    ...
    try {
        ...
        for (const item of items) {
            if (item.type === 'narrativa') {
                ...
                await prisma.entityEntry.updateMany({
                    where: {
...
app.get('/api/tracking/:type/:id', authenticateToken, async (req, res) => {
    ...
    try {
        if (type === 'narrativa') {
            let narrative = null;
            try {
                narrative = await prisma.narrativeCapture.findUnique({
            ...
            const enriched = {
                ...
                peds: (narrative.miss_obj_stra_act_narrative || []).map((p) => ({
                ...
                locations: (narrative.municipality_locality_narrative || []).map((l) => ({
                    municipality: l.cat_municipalities?.name || l.municipality_id.toString(),
                    localities: l.cat_localities?.name || l.locality_id.toString()
                })),
                ods: (narrative.ods_linkage_narrative || []).map((o) => o.cat_ods_linkages?.name || o.ods_linkage_id.toString()),
                timeline: await prisma.narrative_capture_status_histories.findMany({
                    where: { narrative_capture_id: BigInt(id) },
                    include: { users: { select: { name: true, email: true } } },
                    orderBy: { created_at: 'desc' }
                }).then((hist) => hist.map((h) => ({
            ...
            if (!entity)
                return res.status(404).json({ error: 'Captura estadística no encontrada' });
            // Fetch historical entries across periods
            let evolution_data = [];
            try {
                const periods = await prisma.cat_periods.findMany({
                ...
                const periodMap = Object.fromEntries(periods.map((p) => [p.id.toString(), p.name]));
                evolution_data = dbEntries.map((e) => {
                    const row = { period: periodMap[e.period_id?.toString()] || e.cat_periods?.name || '' };
                    e.values.forEach((v) => { row[v.property_id.toString()] = v.value; });
                    return row;
                });
            }
            catch (err) {
...
app.get('/api/export/consolidated/word', authenticateToken, authorize(['super_admin', 'admin', 'secont', 'validador']), async (req, res) => {
    const user = req.user;
    const { periodo, status } = req.query;
    const selectedPeriodYear = parseInt(periodo) || 2025;
    const statusFilter = status || 'approved_secont';
    // Mapeo de año -> narrative_period_id
    const periodYearToId = {
        2023: 2,
        2024: 3,
        2025: 4,
        2026: 1,
    ...
    try {
        ...
        let captures = [];
        try {
            captures = await prisma.narrativeCapture.findMany({
                where: {
...
app.get('/api/export/consolidated/excel', authenticateToken, authorize(['super_admin', 'admin', 'secont', 'validador']), async (req, res) => {
    const user = req.user;
    const { periodo } = req.query;
    const selectedPeriod = parseInt(periodo) || 2026;
    try {
        ...
        const entries = await prisma.entityEntry.findMany({
        ...
        entries.forEach((e) => {
            ...
            rows.forEach((row) => {
                ...
                Object.keys(row).forEach(key => {
                    const prop = e.entity?.properties?.find((p) => p.id.toString() === key);
        ...
        items.forEach((item, sheetIndex) => {
            ...
            ws2.columns = [
                { width: 8 }, // col A (logo)
                ...headerRow.map((h) => ({ width: Math.max(18, Math.min(42, (h?.length || 14) + 4)) }))
            ];
            // LOGO
            if (logoId !== -1) {
                ws2.addImage(logoId, {
...
app.get('/api/stats/global', authenticateToken, authorize(['super_admin', 'admin']), async (req, res) => {
    try {
        ...
        const sectors = await prisma.cat_sectors.findMany();
        const narratives = await prisma.narrativeCapture.findMany({
        ...
        const depsWithCaptures = new Set([
            ...narratives.map((n) => n.dependency_id?.toString()),
            ...entities.map((e) => e.dependency_id?.toString())
        ].filter(Boolean)).size;
        const approvedCount = [
            ...narratives.filter((n) => APPROVED_STATUSES.includes(n.status)),
            ...entities.filter((e) => APPROVED_STATUSES.includes(e.status))
        ].length;
        ...
        const missionStats = missions.map(mission => {
            ...
            const missionNarratives = narratives.filter((n) => {
            ...
            const missionEntities = entities.filter((e) => {
            ...
            const approvedMission = [
                ...missionNarratives.filter((n) => APPROVED_STATUSES.includes(n.status)),
                ...missionEntities.filter((e) => APPROVED_STATUSES.includes(e.status))
            ].length;
            ...
            const depsParticipando = new Set(missionNarratives.map((n) => n.dependency_id).filter(Boolean)).size;
            return {
        ...
        const ranking = dependencies.map(dep => {
            const depNarratives = narratives.filter((n) => n.dependency_id === dep.id);
            const depEntities = entities.filter((e) => e.dependency_id === dep.id);
            const total = depNarratives.length + depEntities.length;
            const approved = [
                ...depNarratives.filter((n) => APPROVED_STATUSES.includes(n.status)),
                ...depEntities.filter((e) => APPROVED_STATUSES.includes(e.status))
            ].length;
        ...
        const inReviewCount = [
            ...narratives.filter((n) => inReviewStatuses.includes(n.status)),
            ...entities.filter((e) => inReviewStatuses.includes(e.status))
        ].length;
...
app.put('/api/admin/entities/:id', authenticateToken, authorize(['super_admin', 'admin', 'safin', 'secont']), async (req, res) => {
    try {
        ...
        if (name !== undefined)
            data.name = name;
        if (dependency_id !== undefined)
            data.dependency_id = BigInt(dependency_id);
        if (is_financial !== undefined)
            data.is_financial = !!is_financial;
        if (second_stage !== undefined)
...
app.delete('/api/admin/entities/:id', authenticateToken, authorize(['super_admin', 'admin', 'safin', 'secont']), async (req, res) => {
    try {
        ...
        logActivity(req.user, "Entidad Eliminada (Soft Delete)", "admin", `Entidad ID ${id} marcada como eliminada`, req);
...
app.put('/api/admin/properties/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        if (catalog_id !== undefined)
            data.catalog_id = catalog_id ? BigInt(catalog_id) : null;
        if (is_required !== undefined)
            data.is_required = !!is_required;
        if (is_additional !== undefined)
            data.is_additional = !!is_additional;
...
app.delete('/api/admin/properties/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        logActivity(req.user, "Columna Eliminada (Soft Delete)", "admin", `Columna ID ${id} marcada como eliminada`, req);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/admin/catalogs/:slug', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        const formattedItems = items.map((item) => {
            let usageCount = 0;
            if (item._count) {
                usageCount = Object.values(item._count).reduce((sum, val) => sum + (Number(val) || 0), 0);
            }
...
app.delete('/api/admin/catalogs/:slug/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        logActivity(req.user, "Ítem de Catálogo Eliminado (Soft Delete)", "catalog_admin", `Ítem ID ${id} del catálogo ${slug} marcado como eliminado`, req);
        res.json({ success: true });
    }
    catch (error) {
...
app.get(['/api/admin/dependencies', '/api/catalogs/dependencies'], authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.json(result.map((d) => ({ ...d, periodo: d.periodo || selectedPeriod })));
    }
    catch (e) {
        res.status(500).json([]);
    }
...
app.post('/api/inbox/update-status', authenticateToken, authorize(['super_admin', 'admin', 'validador']), async (req, res) => {
    ...
    try {
        if (type === 'narrativa') {
            ...
            await prisma.entityEntry.update({
                where: { id: BigInt(id) },
...
app.get('/api/catalogs/municipalities', authenticateToken, async (req, res) => {
    try {
        const municipalities = await prisma.cat_municipalities.findMany({
            orderBy: { name: 'asc' }
        });
...
app.get('/api/catalogs/localities/:municipalityId', authenticateToken, async (req, res) => {
    try {
        ...
        const localities = await prisma.cat_localities.findMany({
...
app.get('/api/catalogs/narrative-titles', authenticateToken, async (req, res) => {
    const pStr = req.query.periodo || "2026";
...
app.get('/api/catalogs/ods', authenticateToken, async (req, res) => {
    ...
    let ods = [];
    try {
        ods = await prisma.odsLinkage.findMany();
    }
    catch (e) {
        ods = [];
    }
...
app.get('/api/catalogs/ppas', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const allPpas = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));
        const filtered = query.length > 0
            ? allPpas.filter(ppa => (ppa.name || "").toLowerCase().includes(query) ||
                (ppa.clave || "").toLowerCase().includes(query) ||
                (ppa.titulo || "").toLowerCase().includes(query)).slice(0, 20)
            : allPpas.slice(0, 20);
...
app.get('/api/catalogs/default-classification', authenticateToken, async (req, res) => {
    try {
        ...
        const allTitles = await prisma.$queryRawUnsafe(`SELECT id, name FROM cat_narrative_titles`);
        const allThemes = await prisma.$queryRawUnsafe(`SELECT id, name FROM cat_narrative_themes`);
        const allSubthemes = await prisma.$queryRawUnsafe(`SELECT id, name FROM cat_narrative_sub_themes`);
        const titleMatch = allTitles.find((t) => (t.name || "").toLowerCase().includes(defaultTitulo.toLowerCase().trim()) ||
            defaultTitulo.toLowerCase().includes((t.name || "").toLowerCase().trim()));
        const themeMatch = allThemes.find((t) => (t.name || "").toLowerCase().includes(defaultTema.toLowerCase().trim()) ||
            defaultTema.toLowerCase().includes((t.name || "").toLowerCase().trim()));
        const subthemeMatch = allSubthemes.find((s) => (s.name || "").toLowerCase().includes(defaultSubtema.toLowerCase().trim()) ||
        ...
        const allowedTitulos = Array.from(new Set(matchingPpas.map((p) => p.titulo).filter(Boolean)));
        const allowedTemasByTitulo = {};
        const allowedSubtemasByTema = {};
        matchingPpas.forEach((p) => {
            if (p.titulo && p.tema) {
                const tituloStr = String(p.titulo);
                if (!allowedTemasByTitulo[tituloStr])
                    allowedTemasByTitulo[tituloStr] = [];
                if (!allowedTemasByTitulo[tituloStr].includes(p.tema))
                    allowedTemasByTitulo[tituloStr].push(p.tema);
            }
            if (p.tema && p.subtema) {
                const temaStr = String(p.tema);
                if (!allowedSubtemasByTema[temaStr])
                    allowedSubtemasByTema[temaStr] = [];
                if (!allowedSubtemasByTema[temaStr].includes(p.subtema))
                    allowedSubtemasByTema[temaStr].push(p.subtema);
            }
        });
        // Match allowed titulos to DB IDs
        const allowedTitleIds = allTitles
            .filter((t) => allowedTitulos.some(at => (t.name || '').toLowerCase().includes(at.toLowerCase().trim()) || at.toLowerCase().includes((t.name || '').toLowerCase().trim())))
            .map((t) => t.id.toString());
...
app.get('/api/catalogs/ppas-by-classification', authenticateToken, async (req, res) => {
    const titulo = (req.query.titulo || "").toLowerCase().trim();
    const tema = (req.query.tema || "").toLowerCase().trim();
    const subtema = (req.query.subtema || "").toLowerCase().trim();
    const titleId = req.query.title_id ? BigInt(req.query.title_id) : null;
    const themeId = req.query.theme_id ? BigInt(req.query.theme_id) : null;
    const periodo = req.query.periodo || "2026";
    // Additional PPAs to exclude (comma-separated names from current session)
    const excludeNames = (req.query.exclude || "").split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
    try {
        const catalogPath = path.join(__dirname, '..', 'ppas_catalog.json');
        if (!fs.existsSync(catalogPath))
            return res.json([]);
        ...
        if (isDbOnline && (titleId || themeId)) {
            try {
                ...
                if (currentPeriod) {
                    ...
                    usedPpaNames = [
                        ...usedPpaNames,
                        ...usedNarratives.map((n) => (n.ppa_name || "").toLowerCase())
                    ];
                }
            }
            catch (dbErr) {
...
app.get('/api/catalogs', authenticateToken, async (req, res) => {
    try {
        const cats = await prisma.catalogs.findMany({
...
app.get('/api/catalogs/narrative-themes', authenticateToken, async (req, res) => {
    const pStr = req.query.periodo || "2026";
    const titleId = req.query.narrative_title_id ? BigInt(req.query.narrative_title_id) : (req.query.title_id ? BigInt(req.query.title_id) : null);
    let themes = [];
    try {
        const where = {};
        if (titleId) {
...
app.get('/api/catalogs/narrative-subthemes', authenticateToken, async (req, res) => {
    const pStr = req.query.periodo || "2026";
    const themeId = req.query.narrative_theme_id ? BigInt(req.query.narrative_theme_id) : (req.query.theme_id ? BigInt(req.query.theme_id) : null);
    let subthemes = [];
    try {
        const where = {};
        if (themeId) {
...
app.get('/api/catalogs/ped/missions', authenticateToken, async (req, res) => {
    try {
        const missions = await prisma.$queryRawUnsafe(`SELECT id, name, code FROM cat_missions ORDER BY code ASC`);
        res.json(missions);
    }
    catch (e) {
        console.error(e);
...
app.get('/api/catalogs/ped/objectives', authenticateToken, async (req, res) => {
    ...
    try {
        let objectives;
        if (missionId) {
            objectives = await prisma.$queryRawUnsafe(`SELECT id, name, code, mission_id FROM cat_objectives WHERE mission_id = ? ORDER BY code ASC`, parseInt(missionId));
        }
        else {
            objectives = await prisma.$queryRawUnsafe(`SELECT id, name, code, mission_id FROM cat_objectives ORDER BY code ASC`);
        }
        res.json(objectives);
    }
    catch (e) {
...
app.get('/api/catalogs/ped/strategies', authenticateToken, async (req, res) => {
    ...
    try {
        let strategies;
        if (objectiveId) {
            strategies = await prisma.$queryRawUnsafe(`SELECT id, name, code, objective_id FROM cat_narrative_strategies WHERE objective_id = ? ORDER BY code ASC`, parseInt(objectiveId));
        }
        else {
            strategies = await prisma.$queryRawUnsafe(`SELECT id, name, code, objective_id FROM cat_narrative_strategies ORDER BY code ASC`);
        }
        res.json(strategies);
    }
    catch (e) {
...
app.get('/api/catalogs/ped/action-lines', authenticateToken, async (req, res) => {
    ...
    try {
        let actionLines;
        if (strategyId) {
            actionLines = await prisma.$queryRawUnsafe(`SELECT id, name, code, narrative_strategy_id FROM cat_action_lines WHERE narrative_strategy_id = ? ORDER BY code ASC`, parseInt(strategyId));
        }
        else {
            actionLines = await prisma.$queryRawUnsafe(`SELECT id, name, code, narrative_strategy_id FROM cat_action_lines ORDER BY code ASC`);
        }
...
app.post('/api/catalogs/:type', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        if (!model)
            return res.status(404).json({ error: "Catálogo no encontrado o no editable vía genérica" });
        const data = { ...req.body };
        delete data.id;
        const item = await model.create({
            data: data
...
app.delete('/api/catalogs/:type/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        logActivity(req.user, "Ítem de Catálogo Eliminado (Soft Delete)", "catalog", `Ítem ID ${id} del catálogo ${type} marcado como eliminado`, req);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/catalogs/ppas-types', authenticateToken, async (req, res) => {
    try {
        const types = await prisma.cat_ppas_types.findMany({
            orderBy: { id: 'asc' }
        });
        res.json(types);
    }
...
app.post('/api/narratives', authenticateToken, async (req, res) => {
    const data = req.body;
    const user = req.user;
    ...
    try {
        ...
        if (data.peds && Array.isArray(data.peds)) {
            for (const ped of data.peds) {
                ...
                if (m && o && s && a) {
                    await prisma.miss_obj_stra_act_narrative.create({
                        data: {
        ...
        if (data.locations && Array.isArray(data.locations)) {
            for (const loc of data.locations) {
                ...
                if (mun && lcl) {
                    await prisma.municipality_locality_narrative.create({
                        data: {
                            narrative_capture_id: narrative.id,
                            municipality_id: mun,
                            locality_id: lcl
...
app.put('/api/narratives/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        console.log(`[DEBUG] ppa_name: "${data.ppa_name}", new_ppa_name: "${data.new_ppa_name}"`);
        const narrativeId = parseInt(narrativeIdParam);
        const narrative = await prisma.narrativeCapture.update({
            where: { id: narrativeId },
            data: {
                ppa_name: data.ppa_name,
                new_ppa_name: data.new_ppa_name || null,
        ...
        if (data.peds && Array.isArray(data.peds)) {
            await prisma.miss_obj_stra_act_narrative.deleteMany({
        ...
        if (data.locations && Array.isArray(data.locations)) {
            await prisma.municipality_locality_narrative.deleteMany({
                where: { narrative_capture_id: BigInt(narrativeId) }
            });
            for (const loc of data.locations) {
                const mun = safeBigInt(loc.municipality_id);
...
async function recordStatusHistory(narrativeId, status, observations, userId) {
    try {
...
async function recordEntityStatusHistory(entityId, status, observations, userId) {
    try {
        await prisma.entity_status_histories.create({
            data: {
                entity_id: entityId,
                status,
                observations: observations || '',
...
app.get('/api/admin/corte-status', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const firstCount = await prisma.$queryRawUnsafe(`SELECT COUNT(*) as count FROM narrative_captures_first_final`);
        const secondCount = await prisma.$queryRawUnsafe(`SELECT COUNT(*) as count FROM narrative_captures_second_final`);
        // Obtener última fecha de actualización de las tablas finales
        const lastUpdated = await prisma.$queryRawUnsafe(`
...
app.post('/api/admin/trigger-second-corte', authenticateToken, authorize(['super_admin']), async (req, res) => {
    const user = req.user;
    try {
...
app.get('/api/dashboard/executive-summary', authenticateToken, authorize(['super_admin', 'admin', 'safin', 'secont']), async (req, res) => {
    try {
        ...
        const pt_safin = ppas.filter(p => ['draft', 'historical__in_capture_'].includes(p.status)).length;
        const ev_safin = ppas.filter(p => p.status === 'under_validation_semaig').length;
        const ap_safin = ppas.filter(p => ['approved_semaig', 'under_validation_secont', 'approved_secont', 'finalized'].includes(p.status)).length;
        const pt_secont = ppas.filter(p => p.status === 'under_validation_secont').length;
        const ap_secont = ppas.filter(p => ['approved_secont', 'finalized', 'historical__revised_'].includes(p.status)).length;
        const ob_safsec = ppas.filter(p => ['with_observations_semaig', 'with_observations_secont'].includes(p.status)).length;
...
app.get('/api/history/snapshots', authenticateToken, authorize(['super_admin', 'admin', 'safin', 'secont']), async (req, res) => {
    try {
        ...
        if (type === 'narrativa') {
            ...
            const rows = await prisma.$queryRawUnsafe(`SELECT * FROM ${table} ORDER BY id DESC`);
...
app.get('/api/history/activity-logs', authenticateToken, authorize(['super_admin', 'admin', 'safin', 'secont']), async (req, res) => {
    try {
...
app.get('/api/narratives/inbox', authenticateToken, async (req, res) => {
    const user = req.user;
    const pStr = req.query.periodo || '2026';
    const selectedPeriod = parseInt(pStr) || 2026;
    const periodId = selectedPeriod === 2026 ? 5 : 4;
    const isSafin = user?.roles?.some((r) => ['safin', 'admin', 'super_admin'].includes(r.toLowerCase()));
    const isSecont = user?.roles?.some((r) => ['secont', 'validador'].includes(r.toLowerCase()));
    const isAdmin = user?.roles?.some((r) => ['super_admin', 'admin'].includes(r.toLowerCase()));
...
app.post('/api/narratives/:id/submit', authenticateToken, async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    fs.appendFileSync('./debug_notify.log', `[${new Date().toISOString()}] POST /api/narratives/${id}/submit called by ${user.email}\n`);
    try {
        const narrative = await prisma.narrativeCapture.findUnique({ where: { id } });
        if (!narrative)
            return res.status(404).json({ error: 'Narrativa no encontrada.' });
...
app.post('/api/narratives/:id/approve-safin', authenticateToken, authorize(['safin', 'admin', 'super_admin']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
...
app.post('/api/narratives/:id/observe-safin', authenticateToken, authorize(['safin', 'admin', 'super_admin']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const { observations } = req.body;
    const editorId = user.id >= 700 || user.id === 105 ? null : BigInt(user.id);
    if (!observations)
        return res.status(400).json({ error: 'Las observaciones son obligatorias para devolver.' });
    try {
...
app.post('/api/narratives/:id/send-to-secont', authenticateToken, authorize(['safin', 'admin', 'super_admin']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const editorId = user.id >= 700 || user.id === 105 ? null : BigInt(user.id);
    try {
...
app.post('/api/narratives/:id/approve-secont', authenticateToken, authorize(['secont', 'validador', 'admin', 'super_admin']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const { observations } = req.body;
...
app.post('/api/narratives/:id/observe-secont', authenticateToken, authorize(['secont', 'validador', 'admin', 'super_admin']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const { observations } = req.body;
    const editorId = (Number(user.id) >= 700 || Number(user.id) === 105) ? null : BigInt(user.id);
...
// ══════════════════════════════════════════════════════════════════════
app.post('/api/entities/:id/approve-safin', authenticateToken, authorize(['safin', 'admin', 'super_admin', 'validador']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const { observations } = req.body;
    const editorId = user.id >= 700 || user.id === 105 ? null : BigInt(user.id);
...
app.post('/api/entities/:id/observe-safin', authenticateToken, authorize(['safin', 'admin', 'super_admin', 'validador']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const { observations } = req.body;
    const editorId = user.id >= 700 || user.id === 105 ? null : BigInt(user.id);
...
app.post('/api/entities/:id/approve-secont', authenticateToken, authorize(['secont', 'validador', 'admin', 'super_admin']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const { observations } = req.body;
    const editorId = (Number(user.id) >= 700 || Number(user.id) === 105) ? null : BigInt(user.id);
...
app.post('/api/entities/:id/observe-secont', authenticateToken, authorize(['secont', 'validador', 'admin', 'super_admin']), async (req, res) => {
    const user = req.user;
    const id = BigInt(String(req.params.id));
    const { observations } = req.body;
    const editorId = (Number(user.id) >= 700 || Number(user.id) === 105) ? null : BigInt(user.id);
...
app.get('/api/narratives/:id/history', authenticateToken, async (req, res) => {
    const id = BigInt(String(req.params.id));
    try {
        const history = await prisma.narrative_capture_status_histories.findMany({
...
app.get('/api/narratives/my-captures', authenticateToken, async (req, res) => {
    try {
        const user = req.user;
        const pStr = req.query.periodo || "2026";
        const selectedPeriod = parseInt(pStr) || 2026;
...
// --- EXPORTACIÓN A WORD ---
app.get('/api/export/word/narrative/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const user = req.user;
    try {
        logActivity(user, "Reporte Word (Narrativa Individual)", "word", `Se exportó la narrativa ID: ${id}`);
        const capture = await prisma.narrativeCapture.findUnique({
            where: { id: parseInt(id) },
...
app.get('/api/export/pdf/narrative/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const user = req.user;
    try {
...
app.get('/api/export/word/:missionId', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const mId = parseInt(missionId) || 0;
        const pStr = req.query.periodo || "2026";
        try {
            ...
            if (captures.length > 0)
                missionName = `Misión ID: ${mId} (${pStr})`;
        }
        catch (dbError) {
...
app.get('/api/export/excel/:entityId', authenticateToken, async (req, res) => {
    const { entityId } = req.params;
    const periodo = parseInt(req.query.periodo || "2026");
    const user = req.user;
    try {
        ...
        if (entries.length > 0) {
            for (const entry of entries) {
                const row = properties.map((prop) => {
                    const val = entry.values?.find((v) => v.property_id.toString() === prop.id.toString());
        ...
        const headerRow = properties.map((p) => p.column_name || p.name);
        const colCount = headerRow.length;
...
app.get('/api/tracking/observations/count', authenticateToken, async (req, res) => {
    try {
        const user = req.user;
        const selectedPeriod = parseInt(req.query.periodo || "2026");
...
app.get('/api/users', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        const allUserHasRoles = await prisma.userHasRole.findMany({
            where: {
                model_id: { in: userIds },
        ...
        const rolesByUserId = allUserHasRoles.reduce((acc, ur) => {
...
app.post('/api/users', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        logActivity(req.user, "Usuario Creado", "user_admin", `Se creó el usuario SQL ${name}`);
...
app.put('/api/users/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        ...
        const updatedUser = await prisma.$transaction(async (tx) => {
            ...
            if (dependency_id)
                updateData.dependency_id = BigInt(dependency_id);
            if (status)
                updateData.is_active = status === 'Activo';
        ...
        logActivity(req.user, "Usuario Editado", "user_admin", `Se modificó configuración SQL de ${name}`);
...
app.patch('/api/users/:id/status', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const userId = BigInt(req.params.id);
        ...
        logActivity(req.user, "Estado de Usuario Modificado", "user_admin", `Se cambió el estado SQL de ${user.name} a ${status}`);
...
app.put('/api/users/:id/status', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const userId = BigInt(req.params.id);
...
app.delete('/api/users/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const userId = BigInt(req.params.id);
        if (userId === BigInt(req.user.id) || userId === BigInt(1000)) {
        ...
        logActivity(req.user, "Usuario Eliminado (Soft Delete)", "user_admin", `El usuario SQL ${user.name} fue marcado como eliminado`, req);
...
app.get('/api/dependencies', authenticateToken, authorize(['super_admin', 'admin', 'safin', 'secont']), async (req, res) => {
    console.log(`[${new Date().toISOString()}] GET /api/dependencies - User:`, req.user?.name, "Roles:", req.user?.roles);
    try {
        const dependencies = await prisma.dependency.findMany({
            where: { deleted_at: null },
            include: {
...
app.post('/api/dependencies/import-excel', authenticateToken, authorize(['super_admin']), upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        allSectors.forEach(s => {
            ...
            if (s.acronym)
                sectorMap.set(s.acronym.toLowerCase().trim(), s.id);
        });
        ...
        logActivity(req.user, "Sincronización de Dependencias", "catalog_admin", `Importación finalizada: ${result.created} nuevas, ${result.updated} actualizadas`);
        res.json({
...
app.post('/api/dependencies', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const { name, code, sector_id } = req.body;
        if (!name || !name.trim())
            return res.status(400).json({ error: "El nombre de la dependencia es requerido." });
        const existing = await prisma.dependency.findFirst({
            where: { name: { equals: name.trim() } }
        });
        if (existing)
            return res.status(400).json({ error: "Ya existe una dependencia con ese nombre." });
        ...
        logActivity(req.user, "Dependencia Creada", "user_admin", `Nueva dependencia: ${newDep.name} (${newDep.acronym})`);
...
app.put('/api/dependencies/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const depId = BigInt(req.params.id);
        const { name, code, sector_id } = req.body;
        if (!name || !name.trim())
        ...
        logActivity(req.user, "Dependencia Actualizada", "user_admin", `ID ${depId}: ${updatedDep.name}`);
...
app.delete('/api/dependencies/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const depId = BigInt(req.params.id);
        ...
        logActivity(req.user, "Dependencia Eliminada (Soft Delete)", "user_admin", `Dependencia marcada como eliminada: ${removedDep.name}`, req);
...
app.get('/api/users/export-csv', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const users = await prisma.user.findMany({
            include: { dependency: true }
        });
...
app.post('/api/users/import-csv', authenticateToken, authorize(['super_admin']), express.text({ type: 'text/csv', limit: '5mb' }), async (req, res) => {
    try {
        ...
        deps.forEach((d) => {
            ...
            if (d.code)
                depsByName[d.code.trim().toLowerCase()] = d;
        });
        const lines = req.body.split('\n').filter(Boolean);
        if (lines.length < 2)
            return res.status(400).json({ error: "CSV vacío o sin filas de datos." });
        const headerRaw = lines[0];
        if (!headerRaw)
            return res.status(400).json({ error: "CSV sin cabecera." });
        const headerLine = headerRaw.split(',').map((h) => h.trim().replace(/"/g, '').toLowerCase());
        const idIdx = headerLine.indexOf('id');
        const depNameIdx = headerLine.findIndex((h) => h.includes('dependencia_nombre'));
        const depIdIdx = headerLine.findIndex((h) => h === 'dependencia_id');
        if (idIdx === -1)
            return res.status(400).json({ error: "El CSV debe tener una columna 'ID'." });
        let updated = 0;
        const errors = [];
        for (let i = 1; i < lines.length; i++) {
            ...
            let matchedDepId = null;
            if (depName) {
                const matched = depsByName[depName.toLowerCase()];
                if (matched)
                    matchedDepId = BigInt(matched.id);
            }
            else if (!isNaN(depId)) {
        ...
        logActivity(req.user, "Importación Masiva CSV", "user_admin", `${updated} usuarios actualizados por CSV`, req);
...
app.get('/api/logs', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
...
// ==========================================
app.get('/api/admin/recycle-bin/:type', authenticateToken, authorize(['super_admin', 'admin']), async (req, res) => {
    const { type } = req.params;
    try {
        if (type === 'narratives') {
            const deleted = await prisma.narrativeCapture.findMany({
                where: { deleted_at: { not: null } },
                include: { dependency: true, cat_narrative_periods: true },
                orderBy: { deleted_at: 'desc' }
            });
            const items = deleted.map((n) => ({
                id: n.id,
                name: n.ppa_name,
                acronym: `PPA-${n.id}`,
                type: 'Narrativa',
            ...
            const deleted = await prisma.dependency.findMany({
                where: { deleted_at: { not: null } },
                orderBy: { deleted_at: 'desc' }
            });
...
app.post('/api/admin/recycle-bin/:type/:id/restore', authenticateToken, authorize(['super_admin', 'admin']), async (req, res) => {
    const { type, id } = req.params;
    const user = req.user;
    try {
        if (type === 'narratives') {
            const restored = await prisma.narrativeCapture.update({
                where: { id: BigInt(id) },
            ...
            const restored = await prisma.dependency.update({
                where: { id: BigInt(id) },
                data: { deleted_at: null, updated_at: new Date() }
            });
...
// ============================================================
// GET all periods
app.get('/api/periods', authenticateToken, async (req, res) => {
    try {
        const periods = await prisma.cat_narrative_periods.findMany({ orderBy: { id: 'asc' } });
        res.json(periods);
    }
    catch (error) {
...
app.get('/api/periods/active', authenticateToken, async (req, res) => {
    try {
        const active = await prisma.cat_narrative_periods.findFirst({ orderBy: { id: 'desc' } });
        if (!active)
...
// POST create period
app.post('/api/periods', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const { year, name, description } = req.body;
        if (!year)
            return res.status(400).json({ error: "El año es requerido." });
        const newPeriod = await prisma.cat_narrative_periods.create({
            data: { year: String(year), name: name || `Informe de Gobierno ${year}`, description: description || '', is_active: false }
        });
        logActivity(req.user, "Periodo Creado", "user_admin", `Periodo ${newPeriod.name} creado`);
...
app.put('/api/periods/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const periodId = parseInt(req.params.id);
        const { name, description } = req.body;
        const updated = await prisma.cat_narrative_periods.update({
            where: { id: periodId },
            data: { name, description }
        });
        logActivity(req.user, "Periodo Actualizado", "user_admin", `Periodo ${updated.name} actualizado`);
        res.json(updated);
    }
    catch (error) {
...
app.patch('/api/periods/:id/activate', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const periodId = parseInt(req.params.id);
        await prisma.cat_narrative_periods.updateMany({ data: { is_active: false } });
        const activated = await prisma.cat_narrative_periods.update({
            where: { id: periodId },
            data: { is_active: true }
        });
        logActivity(req.user, "Periodo Activado", "user_admin", `Ciclo activo cambiado a: ${activated.name}`);
        res.json(activated);
    }
    catch (error) {
...
app.patch('/api/periods/:id/stages/:stageId', authenticateToken, authorize(['super_admin']), async (req, res) => {
    res.status(501).json({ error: "La gestión de etapas individuales no está implementada en la base de datos." });
});
// DELETE period
app.delete('/api/periods/:id', authenticateToken, authorize(['super_admin']), async (req, res) => {
    try {
        const periodId = parseInt(req.params.id);
        await prisma.cat_narrative_periods.update({
            where: { id: periodId },
            data: { deleted_at: new Date() }
        });
        logActivity(req.user, "Periodo Eliminado (Soft Delete)", "user_admin", `Periodo ID ${periodId} marcado como eliminado`, req);
...
app.get('/api/secont/narrativas', authenticateToken, authorize(['secont', 'super_admin']), async (req, res) => {
    try {
        const pStr = req.query.periodo || '2026';
        const period = await prisma.cat_narrative_periods.findFirst({ where: { year: pStr } });
        const secontStatuses = ['under_validation_secont', 'with_observations_secont', 'approved_secont'];
        const narratives = period ? await prisma.narrativeCapture.findMany({
...
app.get('/api/secont/anexos', authenticateToken, authorize(['secont', 'super_admin']), async (req, res) => {
    try {
...
app.post('/api/secont/review/:type/:id', authenticateToken, authorize(['secont', 'super_admin']), async (req, res) => {
    try {
...
app.post('/api/inbox/send-to-secont', authenticateToken, authorize(['super_admin', 'admin', 'validador']), async (req, res) => {
    try {
...
app.get('/api/secont/stats', authenticateToken, authorize(['secont', 'super_admin', 'admin']), async (req, res) => {
    try {
        const pStr = req.query.periodo || '2026';
        const period = await prisma.cat_narrative_periods.findFirst({ where: { year: pStr } });
        if (!period)
            return res.json({ pendientes: 0, aprobados: 0, observaciones: 0, total: 0 });
        const [pendientes, aprobados, observaciones] = await Promise.all([
            prisma.narrativeCapture.count({ where: { narrative_period_id: period.id, status: 'under_validation_secont' } }),
            prisma.narrativeCapture.count({ where: { narrative_period_id: period.id, status: 'approved_secont' } }),
            prisma.narrativeCapture.count({ where: { narrative_period_id: period.id, status: 'with_observations_secont' } }),
        ]);
...
app.listen(PORT, () => {
    ...
    if (!TBOT_TOKEN) {
        ...
        const fmtDate = (iso) => iso
            ...
            : '—';
        const emoji = { login: '🔑', logout: '🚪', create: '➕', update: '✏️', delete: '🗑️', export: '📤', user_admin: '🛡️' };
        const getStatus = async () => {
            const usersCount = await prisma.user.count();
            const narrCount = await prisma.narrativeCapture.count();
            const pend = await prisma.narrativeCapture.count({ where: { status: { in: ['submitted', 'pendiente', 'enviado'] } } });
            return [
        ...
        const getPending = async () => {
            const pend = await prisma.narrativeCapture.findMany({
                where: { status: { in: ['submitted', 'pendiente', 'enviado'] } },
                take: 15
            });
            if (!pend.length)
                return '✅ *No hay narrativas pendientes de validación.*';
            const lines = pend.map((n, i) => `${i + 1}. *${n.ppa_name || 'Sin título'}*`);
            return [`⏳ *PENDIENTES (${pend.length})*`, ``, ...lines].join('\n');
        };
        const getUsers = async () => {
            const usersCount = await prisma.user.count();
            const activeCount = await prisma.user.count({ where: { is_active: true } });
            return [
        ...
        const getDeps = async () => {
            const depsCount = await prisma.dependency.count();
        ...
        const getSecont = async () => {
            const pend = await prisma.narrativeCapture.count({ where: { status: { contains: 'secont' } } });
            return [
                `🛡️ *AUDITORÍA secont*`, ``,
        ...
        tbot.onText(/\/start/, msg => guard(msg.chat.id, () => tbot.sendMessage(msg.chat.id, `👋 *Bot SEPLAN Captura Informe V2*\n\n${MENU}`, { parse_mode: 'Markdown' })));
        tbot.onText(/\/ayuda/, msg => guard(msg.chat.id, () => tbot.sendMessage(msg.chat.id, `📋 *Comandos disponibles:*\n\n${MENU}`, { parse_mode: 'Markdown' })));
        tbot.onText(/\/status/, async (msg) => guard(msg.chat.id, async () => tbot.sendMessage(msg.chat.id, await getStatus(), { parse_mode: 'Markdown' })));
        tbot.onText(/\/pendientes/, async (msg) => guard(msg.chat.id, async () => tbot.sendMessage(msg.chat.id, await getPending(), { parse_mode: 'Markdown' })));
        tbot.onText(/\/secont/, async (msg) => guard(msg.chat.id, async () => tbot.sendMessage(msg.chat.id, await getSecont(), { parse_mode: 'Markdown' })));
        tbot.onText(/\/usuarios/, async (msg) => guard(msg.chat.id, async () => tbot.sendMessage(msg.chat.id, await getUsers(), { parse_mode: 'Markdown' })));
        tbot.onText(/\/dependencias/, async (msg) => guard(msg.chat.id, async () => tbot.sendMessage(msg.chat.id, await getDeps(), { parse_mode: 'Markdown' })));
        tbot.onText(/\/logs/, async (msg) => guard(msg.chat.id, async () => tbot.sendMessage(msg.chat.id, await getLogs(), { parse_mode: 'Markdown' })));
