...
async function main() {
    ...
    const themes = new Map(); // tema -> titulo
    const subthemes = new Map(); // subtema -> tema
    const ppaTypes = new Set();
    const fundings = new Set(sourcesFallback);
    const beneficiaries = new Set(beneficiariesFallback);
    const budgetPrograms = new Map();
    for (const p of ppas) {
        if (p.titulo)
            titles.add(p.titulo);
        if (p.tema && p.titulo)
            themes.set(p.tema, p.titulo);
        if (p.subtema && p.tema)
            subthemes.set(p.subtema, p.tema);
        if (p.tipo_ppa)
            ppaTypes.add(p.tipo_ppa);
        if (p.fuente)
            fundings.add(p.fuente);
        if (p.tipo_beneficiario)
            beneficiaries.add(p.tipo_beneficiario);
    ...
    for (const titleName of titles) {
        try {
            ...
            console.error("Error with title", titleName);
        }
    }
    let thCode = 1;
    for (const [themeName, titleName] of themes.entries()) {
        try {
            ...
            console.error("Error with theme", themeName);
        }
    }
    let sCode = 1;
    for (const [subthemeName, themeName] of subthemes.entries()) {
        try {
            const theme = await prisma.narrativeTheme.findFirst({ where: { name: themeName.toString() } });
            const sub = await prisma.cat_narrative_sub_themes.findFirst({ where: { name: subthemeName.toString() } });
            if (theme && !sub) {
                await prisma.cat_narrative_sub_themes.create({
                    data: { name: subthemeName.toString(), code: sCode++, narrative_theme_id: theme.id }
                });
            }
        }
        catch (e) {
            console.error("Error with subtheme", subthemeName);
        }
    }
    for (const type of ppaTypes) {
        try {
            const pt = await prisma.cat_ppas_types.findFirst({ where: { name: type.toString() } });
            if (!pt)
                await prisma.cat_ppas_types.create({ data: { name: type.toString(), acronym: type.toString().substring(0, 5) } });
        }
        catch (e) { /* ignore */ }
    }
    for (const fund of fundings) {
        try {
            const fs = await prisma.cat_narrative_financing_sources.findFirst({ where: { name: fund.toString() } });
            if (!fs)
                await prisma.cat_narrative_financing_sources.create({ data: { name: fund.toString() } });
        }
        catch (e) { /* ignore */ }
    }
    for (const ben of beneficiaries) {
        try {
            const bt = await prisma.cat_narrative_beneficiary_types.findFirst({ where: { name: ben.toString() } });
            if (!bt)
                await prisma.cat_narrative_beneficiary_types.create({ data: { name: ben.toString() } });
        }
        catch (e) { /* ignore */ }
    }
...
//# sourceMappingURL=seed_catalogs.js.map